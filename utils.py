...
#  nodes要改成key points的某些features， centers直接改成key points的坐标
def process_pair(path):
    ...
    """
    data1 = json.load(open(path[0]))  # load first json file, dict datas
    data2 = json.load(open(path[1]))  # load second json file, dict datas
    data={}
    data["centers_1"]=data1["centers"]
    data["nodes_1"]=data1["nodes"] # data["nodes_1"]保存了kp的描述子
...
# created by cjf
def process_rskdd_pair(path):
  """
  Reading rskdd keypoints and descriptors files 
  Return data: Dictionary with data
  """
...
# visualize point cloud, for debug
