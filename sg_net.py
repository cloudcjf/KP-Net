...
class SG(torch.nn.Module):      
    ...
    # 所有卷积的input维度都乘以2是因为EdgeConv的时候有concatenate操作，特征维度翻倍
    def setup_layers(self):
        """
        Creating the layers.
        """
        print("setup layers of node embedding")
        self.calculate_bottleneck_features() # self.feature_count = 16
    ...
    def dgcnn_conv_pass(self, x):
        ...
        xyz = self.dgcnn_s_conv1(xyz)                 #Bx64xNxk
        xyz1 = xyz.max(dim=-1, keepdim=False)[0]      #Bx64xN
        xyz = dgcnn.get_graph_feature(xyz1, k=self.k) #Bx128xNxk
        xyz = self.dgcnn_s_conv2(xyz)                 #Bx64xNxk
        xyz2 = xyz.max(dim=-1, keepdim=False)[0]      #Bx64xN
        xyz = dgcnn.get_graph_feature(xyz2, k=self.k) #Bx128xNxk
        xyz = self.dgcnn_s_conv3(xyz)                 #Bx32xNxk
        xyz3 = xyz.max(dim=-1, keepdim=False)[0]      #Bx32xN
        ...
        x = torch.cat((xyz3, sem3), dim=1)          # Bx64xN
        # x = self.dgcnn_conv_all(x)
        x = self.dgcnn_conv_end(x)                  # Bx32xN

        x = x.permute(0, 2, 1)                      # BxNx32
        # print(x.shape)
        return x
    ...
    def forward(self, data):
        ...
        # BxNx32
        abstract_features_1 = self.dgcnn_conv_pass(features_1) 
        abstract_features_2 = self.dgcnn_conv_pass(features_2) 
...
class SGTrainer(object):
    ...
    def setup_model(self,train=True):
        ...
        if (self.args.continue_train) and self.model_pth != "": # continue train with the exist model
            print("continue train on model: ", self.model_pth)
